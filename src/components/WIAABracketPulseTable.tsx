...
export default function WIAABracketPulseTable({ division }: WIAABracketPulseTableProps) {
  ...
  if (division === "1") {
    ...
    const renderD1Region = (regionName: string, regionNum: string) => {
      ...
      // Regional Semis → Regional Finals: pick by RegionalFinals
      const r1Winners = matchupsD1.map(([s1, s2]) => {
        const t1 = getTeamBySeed(s1), t2 = getTeamBySeed(s2);
        return (t1 && t2) ? getProjectedWinner([t1, t2], "RegionalFinals") : t1 || t2;
      });

      // Regional Finals → Sectional Semi: pick by SectionalSemi
      const r2Winners = [
        getProjectedWinner([r1Winners[0], r1Winners[1]].filter(Boolean) as Team[], "SectionalSemi"),
        getProjectedWinner([r1Winners[2], r1Winners[3]].filter(Boolean) as Team[], "SectionalSemi"),
        getProjectedWinner([r1Winners[4], r1Winners[5]].filter(Boolean) as Team[], "SectionalSemi"),
        getProjectedWinner([r1Winners[6], r1Winners[7]].filter(Boolean) as Team[], "SectionalSemi"),
      ];

      // Sectional Semi → Sectional Final: pick by SectionalFinal
      const r3Winners = [
        getProjectedWinner([r2Winners[0], r2Winners[1]].filter(Boolean) as Team[], "SectionalFinal"),
        getProjectedWinner([r2Winners[2], r2Winners[3]].filter(Boolean) as Team[], "SectionalFinal"),
      ];
        ...
        <div key={regionName} style={CARD_STYLE}>
          ...
          <div style={{ background: "white", padding: 20, overflowX: "auto" }}>
            <div style={{ position: "relative", height: totalHeight, minWidth: TEAM_WIDTH * 5 + CONNECTOR_WIDTH * 4 }}>
              ...
              {/* Regional Finals */}
              {r1Winners.map((w, i) => <div key={`r2-${i}`} style={{ position: "absolute", top: r32Positions[i] - TEAM_HEIGHT / 2, left: TEAM_WIDTH + CONNECTOR_WIDTH }}><TeamSlot team={w} prob={w?.RegionalFinals} /></div>)}
              ...
              {/* Sectional Semi */}
              {r2Winners.map((w, i) => <div key={`r3-${i}`} style={{ position: "absolute", top: r16Positions[i] - TEAM_HEIGHT / 2, left: (TEAM_WIDTH + CONNECTOR_WIDTH) * 2 }}><TeamSlot team={w} prob={w?.SectionalSemi} /></div>)}
              ...
              {/* Sectional Final */}
              {r3Winners.map((w, i) => <div key={`r4-${i}`} style={{ position: "absolute", top: r8Positions[i] - TEAM_HEIGHT / 2, left: (TEAM_WIDTH + CONNECTOR_WIDTH) * 3 }}><TeamSlot team={w} prob={w?.SectionalFinal} /></div>)}
  ...
  const renderSectional = (sectionalName: string, regionAName: string, regionBName: string) => {
    ...
    // Region A — Regional Semis → Regional Finals: pick by RegionalFinals
    const rsA = matchupsWIAA.map(([s1, s2]) => {
      const t1 = getA(s1), t2 = getA(s2);
      return (t1 && t2) ? getProjectedWinner([t1, t2], "RegionalFinals") : t1 || t2;
    });
    // Region A — Regional Finals → Sectional Semi: pick by SectionalSemi
    const rcA = [
      getProjectedWinner([rsA[0], rsA[1]].filter(Boolean) as Team[], "SectionalSemi"),
      getProjectedWinner([rsA[2], rsA[3]].filter(Boolean) as Team[], "SectionalSemi"),
    ];
    // Region A — Sectional Semi → Sectional Final: pick by SectionalFinal
    const winnerA = getProjectedWinner(rcA.filter(Boolean) as Team[], "SectionalFinal");

    // Region B — Regional Semis → Regional Finals: pick by RegionalFinals
    const rsB = matchupsWIAA.map(([s1, s2]) => {
      const t1 = getB(s1), t2 = getB(s2);
      return (t1 && t2) ? getProjectedWinner([t1, t2], "RegionalFinals") : t1 || t2;
    });
    // Region B — Regional Finals → Sectional Semi: pick by SectionalSemi
    const rcB = [
      getProjectedWinner([rsB[0], rsB[1]].filter(Boolean) as Team[], "SectionalSemi"),
      getProjectedWinner([rsB[2], rsB[3]].filter(Boolean) as Team[], "SectionalSemi"),
    ];
    // Region B — Sectional Semi → Sectional Final: pick by SectionalFinal
    const winnerB = getProjectedWinner(rcB.filter(Boolean) as Team[], "SectionalFinal");
      ...
      <div key={sectionalName} style={CARD_STYLE}>
        ...
        <div style={{ background: "white", padding: 20, overflowX: "auto" }}>
          <div style={{ position: "relative", height: totalH, minWidth: TEAM_WIDTH * 5 + CONNECTOR_WIDTH * 4 }}>
            ...
            {rsA.map((w, i) => <div key={`rfa-${i}`} style={{ position: "absolute", top: r32[i] - TEAM_HEIGHT/2, left: TEAM_WIDTH + CONNECTOR_WIDTH }}><TeamSlot team={w} prob={w?.RegionalFinals} /></div>)}
            ...
            {rcA.map((w, i) => <div key={`ssa-${i}`} style={{ position: "absolute", top: s16[i] - TEAM_HEIGHT/2, left: (TEAM_WIDTH+CONNECTOR_WIDTH)*2 }}><TeamSlot team={w} prob={w?.SectionalSemi} /></div>)}
            ...
            <div style={{ position: "absolute", top: sectPos - TEAM_HEIGHT/2, left: (TEAM_WIDTH+CONNECTOR_WIDTH)*3 }}><TeamSlot team={winnerA} prob={winnerA?.SectionalFinal} /></div>
            ...
            {rsB.map((w, i) => <div key={`rfb-${i}`} style={{ position: "absolute", top: r32[i] - TEAM_HEIGHT/2 + half, left: TEAM_WIDTH + CONNECTOR_WIDTH }}><TeamSlot team={w} prob={w?.RegionalFinals} /></div>)}
            ...
            {rcB.map((w, i) => <div key={`ssb-${i}`} style={{ position: "absolute", top: s16[i] - TEAM_HEIGHT/2 + half, left: (TEAM_WIDTH+CONNECTOR_WIDTH)*2 }}><TeamSlot team={w} prob={w?.SectionalSemi} /></div>)}
            ...
            <div style={{ position: "absolute", top: sectPos - TEAM_HEIGHT/2 + half, left: (TEAM_WIDTH+CONNECTOR_WIDTH)*3 }}><TeamSlot team={winnerB} prob={winnerB?.SectionalFinal} /></div>
