  ...
  return (
    <>
      ...
      <div className="section-wrapper">
        ...
        {/* UPDATED: Centered, reduced width controls */}
        <div className="mb-6 flex flex-col items-center gap-3">
          ...
          <div className="text-sm text-stone-600 tracking-tight">
            ...
            <span className="font-semibold">{totalTeams}</span> teams. Updated rankings as of{" "}
            {lastUpdated}
          </div>
        </div>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th className="cursor-pointer select-none" onClick={() => handleSort("conference")}>
                      Conference {sortIcon("conference")}
                    </th>
                    <th className="cursor-pointer select-none text-right" onClick={() => handleSort("kenpom_rank")}>
                      KenPom {sortIcon("kenpom_rank")}
                    </th>
                    <th className="cursor-pointer select-none text-right" onClick={() => handleSort("net_ranking")}>
                      NET {sortIcon("net_ranking")}
                    </th>
                    <th className="cursor-pointer select-none text-right" onClick={() => handleSort("record")}>
                ...
                <tbody>
                  {sortedRankings.map((team, index) => (
                    <tr key={`${team.team}-${team.model_rank}`} className={index % 2 === 0 ? "bg-stone-50/40" : "bg-white"}>
                      <td ref={index === 0 ? rankRef : null} className="sticky left-0 z-20 bg-white/90 backdrop-blur-sm font-mono text-sm">
                        {team.model_rank}
                      </td>
                      <td className="sticky z-20 bg-white/90 backdrop-blur-sm font-medium text-stone-900 whitespace-nowrap" style={{ left: rankWidth }}>
                        {team.team}
                      </td>
                      <td className="whitespace-nowrap text-stone-700">{team.conference}</td>
                      <td className="whitespace-nowrap text-right">
                        <span className={getRankTextColor(team.kenpom_rank)}>{team.kenpom_rank}</span>
                      </td>
                      <td className="whitespace-nowrap text-right">
                        <span className={getRankTextColor(team.net_ranking)}>{team.net_ranking}</span>
                      </td>
                  ...
                  {sortedRankings.length === 0 && (
                    <tr>
                      <td colSpan={6} className="text-center py-6 text-stone-500">No teams match your filters.</td>
                    </tr>
                  )}
                </tbody>
              </table>
