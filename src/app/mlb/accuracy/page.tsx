...
// Audit value set on the 13 historical UNDER records generated
// 2026-03-31 through 2026-04-19 while the OU_UNDER_PAUSED flag was
// wired at only 1 of 3 ouPick assignment sites. Records carrying this
// field are display-only — excluded from all live aggregate cover-rate
// calculations and rendered in a dedicated "pre-pause-fix pipeline
// output" section below the main UNDER table with a full explanation.
// See BoundScraper/docs/model_v2/post_v2_backlog.md §12.
const PRE_PAUSE_FIX_AUDIT = "pre_pause_fix_leak_2026-04-20";
      ...
      {open && (
        <div style={{ backgroundColor: "#f9fafb", padding: "20px 24px", borderTop: "1px solid #d6d3d1", fontSize: 14, color: "#44403c", lineHeight: 1.65 }}>
          {mode === "rl" ? (
            ...
            <>
              <p style={{ marginBottom: 12 }}>This page tracks every MLB total (over/under) pick BBMI has made &mdash; with full results logged publicly, unedited, from the first pick of the 2026 season.</p>
              <p style={{ marginBottom: 12 }}><strong>Over picks</strong> are generated when the model projects the total edge {"\u2265"} 1.50 runs above the posted line and passes the production filter cascade (Week 13+ of season, posted total {"<"} 9.0, minimum 20 team games played, non-opener, PREMIUM or ELITE confidence tier).</p>
              <p style={{ marginBottom: 12 }}><strong>Model v2 (2026-04-20) note on the Over number.</strong> Under the corrected filter-parity methodology, Over ships at <strong>67.21% cover on 122 picks</strong> (2024-2025 real-PIT walk-forward through 2025-09-28). The previously published Release 5 figure of 60.4% on 217 picks was computed by a validator whose filter logic diverged from production in three ways: Jun+ seasonal gate vs. production{"'"}s Week-13+ gate, no posted-total {"\u2265"} 9.0 suppression, and no PREMIUM/ELITE tier publish filter. As a result, the validator{"'"}s 217-pick cell did not match production{"'"}s actual shipped picks &mdash; the 60.4% figure described a superset of games that production would never have published. That validator was refactored to import production{"'"}s filter directly, closing the parity gap by construction. <strong>The v2 parameter changes did NOT improve the Over product; the correction revealed that Release 5{"'"}s headline rate was computed against the wrong cell of games.</strong></p>
              <p style={{ fontSize: 12, color: "#78716c", marginTop: 10, marginBottom: 0 }}><strong>Under product:</strong> PAUSED under v2. Real-PIT cover rate 56.76% on 296 picks is below the 60% live-product gate. v2 parameter changes did not produce a cover-rate lift sufficient to relaunch. The prior pause decision stands. Revisit: 2026 season midpoint with a fresh data draw. Past performance does not guarantee future results.</p>
            </>
          )}
        </div>
      )}
...
// ────────────────────────────────────────────────────────────────
// PRE-PAUSE-FIX PIPELINE OUTPUT (audit-only)
// ────────────────────────────────────────────────────────────────

type PrePauseFixRecord = {
  gameId: string;
  date: string;
  homeTeam: string;
  awayTeam: string;
...
function PrePauseFixSection({ records }: { records: PrePauseFixRecord[] }) {
  const [open, setOpen] = useState(false);
  const wins = records.filter(r => r.won === true).length;
  const decided = records.filter(r => r.won !== null).length;
  return (
    <div style={{ maxWidth: 1100, margin: "2.5rem auto 0", backgroundColor: "#fef3c7", borderTop: "1px solid #fde68a", borderRight: "1px solid #fde68a", borderBottom: "1px solid #fde68a", borderLeft: "4px solid #d97706", borderRadius: 6, overflow: "hidden" }}>
      <button type="button" onClick={() => setOpen(p => !p)}
        style={{ width: "100%", padding: "10px 14px", backgroundColor: "transparent", color: "#92400e", fontSize: "0.75rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", border: "none", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span>Pre-pause-fix pipeline output (2026-03-31 through 2026-04-19) &mdash; {records.length} picks</span>
        <span>{open ? "\u25B2" : "\u25BC"}</span>
      </button>
      {open && (
        <div style={{ padding: "1.25rem 1.5rem", fontSize: 13, color: "#78350f", lineHeight: 1.65 }}>
          <p style={{ marginTop: 0, marginBottom: 12 }}>
            These {records.length} UNDER picks were generated by a wiring bug in the pipeline between 2026-03-31 and 2026-04-19. During this window, the <code>OU_UNDER_PAUSED</code> flag that should have prevented UNDER pick generation was only respected at one of three pipeline assignment sites; the other two sites generated picks despite the pause.
          </p>
          <p style={{ marginBottom: 12 }}>
            These picks never reached live subscribers &mdash; a separate deploy-layer issue prevented MLB data from being committed to production during the same period. Both issues were fixed on 2026-04-20 as part of Model v2 release follow-up work. These records are preserved here for audit transparency rather than hidden, and they are excluded from the product&apos;s cover-rate calculations.
          </p>
          <p style={{ marginBottom: 16, fontSize: 12, color: "#92400e" }}>
            Historical record ({decided > 0 ? `${wins} of ${decided} covered, ${((wins / decided) * 100).toFixed(1)}%` : "not yet graded"}). Displayed here only; not aggregated into any UNDER product stat.
          </p>
          <div style={{ overflowX: "auto", marginTop: 8 }}>
            <table style={{ width: "100%", fontSize: 12, borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  <th style={{ ...TH, color: "#78350f", borderBottom: "1px solid #fde68a" }}>Date</th>
                  <th style={{ ...TH, color: "#78350f", borderBottom: "1px solid #fde68a" }}>Matchup</th>
                  <th style={{ ...TH, color: "#78350f", borderBottom: "1px solid #fde68a", textAlign: "right" }}>Vegas Total</th>
                  <th style={{ ...TH, color: "#78350f", borderBottom: "1px solid #fde68a", textAlign: "right" }}>Edge</th>
                  <th style={{ ...TH, color: "#78350f", borderBottom: "1px solid #fde68a", textAlign: "right" }}>Actual</th>
                  <th style={{ ...TH, color: "#78350f", borderBottom: "1px solid #fde68a", textAlign: "center" }}>Cover</th>
                </tr>
              </thead>
              <tbody>
                {records.map(r => (
                  <tr key={r.gameId}>
                    <td style={{ ...TD_MONO, color: "#78350f" }}>{r.date}</td>
                    <td style={{ ...TD, color: "#78350f" }}>{r.awayTeam} @ {r.homeTeam}</td>
                    <td style={{ ...TD_MONO, color: "#78350f", textAlign: "right" }}>{r.vegasTotal != null ? r.vegasTotal.toFixed(1) : "\u2014"}</td>
                    <td style={{ ...TD_MONO, color: "#78350f", textAlign: "right" }}>{r.ouEdge != null ? r.ouEdge.toFixed(2) : "\u2014"}</td>
                    <td style={{ ...TD_MONO, color: "#78350f", textAlign: "right" }}>{r.actualTotal}</td>
                    <td style={{ ...TD_MONO, color: "#78350f", textAlign: "center" }}>
                      {r.won === null ? "\u2014" : r.won ? "\u2713" : "\u2717"}
                    </td>
                  </tr>
                ))}
...
export default function MLBAccuracyPage() {
  ...
  // Excludes records flagged with ouPickAudit === PRE_PAUSE_FIX_AUDIT. Those
  // records are rendered in a dedicated section below (see
  // `ouUnderPrePauseFix`) and are NOT included in aggregate cover-rate
  // calculations — the UNDER product's banner status is "paused" under v2
  // and those 13 picks reflect a pipeline wiring bug, not intended output.
  const ouUnderResults = useMemo(() => {
    return completed
      .filter(g => g.ouPick === "UNDER" && g.ouPickAudit !== PRE_PAUSE_FIX_AUDIT)
      .map(g => {
  ...
  // ── Pre-pause-fix pipeline output (audit-only, not an active product) ──
  // The 13 UNDER picks generated 2026-03-31 through 2026-04-19 while the
  // OU_UNDER_PAUSED wiring bug was live. Preserved for audit transparency;
  // rendered in a dedicated collapsed section and excluded from all
  // aggregates.
  const ouUnderPrePauseFix = useMemo(() => {
    return completed
      .filter(g => g.ouPick === "UNDER" && g.ouPickAudit === PRE_PAUSE_FIX_AUDIT)
      .map(g => {
        const actual = (g.actualHomeScore ?? 0) + (g.actualAwayScore ?? 0);
        const won = actual < (g.vegasTotal ?? 0);
        const push = actual === (g.vegasTotal ?? 0);
        return { ...g, actualTotal: actual, won: push ? null : won, call: "UNDER" as const };
      })
  ...
  // RL mode: restrict to in-cell picks (away team was the Vegas underdog at
  // time of pick). Out-of-cell legacy picks generated by the pre-2026-04-18
  // buggy filter are excluded from all downstream views — table, weekly
  // breakdown, rolling 30, streak, tier breakdown — so the displayed
  // history reflects only picks valid under the corrected product definition.
  const activeResults = mode === "rl" ? rlResults.filter(r => r.inNewCell) : ouAllResults;
    ...
    <>
      ...
      <div className="section-wrapper" style={{ backgroundColor: "#f0efe9", minHeight: "100vh" }}>
        <div className="w-full max-w-[1200px] mx-auto px-6 py-8">
          ...
          {/* ── PRE-PAUSE-FIX PIPELINE OUTPUT ─────────────── */}
          {/* The 13 UNDER picks generated 2026-03-31 through 2026-04-19 while
              the OU_UNDER_PAUSED wiring bug was live. Excluded from all
              aggregates above; rendered here for audit transparency. */}
          {mode === "ou" && ouUnderPrePauseFix.length > 0 && (
            <PrePauseFixSection records={ouUnderPrePauseFix} />
          )}
