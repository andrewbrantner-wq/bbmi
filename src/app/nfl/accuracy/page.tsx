...
import games from "@/data/betting-lines/nfl-games.json";

import NFLLogo from "@/components/NFLLogo";
import { OU_MIN_EDGE as MIN_EDGE_FOR_RECORD, OU_PREMIUM_EDGE as FREE_EDGE_LIMIT, OU_MAX_EDGE as MAX_EDGE_FOR_RECORD } from "@/config/nfl-thresholds";
...
function HighEdgeCallout({ overallWinPct, overallTotal, highEdgeWinPct, highEdgeTotal, eliteEdgeWinPct, eliteEdgeTotal, highEdgeThreshold = FREE_EDGE_LIMIT, eliteEdgeThreshold = 10 }: {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto 2rem", backgroundColor: "#013369", borderRadius: 0, border: "2px solid #013369", overflow: "hidden" }}>
      <style>{`
      ...
      <div className="hec-grid">
        ...
        <div className="hec-cta">
          ...
          <a href="/nfl/picks" style={{ display: "inline-block", backgroundColor: "#ffffff", color: "#1a1a1a", padding: "0.5rem 1.1rem", borderRadius: 7, fontWeight: 800, fontSize: "0.8rem", textDecoration: "none", whiteSpace: "nowrap" }}>
            Unlock — $10 trial →
          </a>
...
function ColDescPortal({ tooltipId, anchorRect, onClose }: { tooltipId: string; anchorRect: DOMRect; onClose: () => void }) {
  ...
  return ReactDOM.createPortal(
    <div ref={el} style={{ position: "fixed", top, left, zIndex: 99999, width: 220, backgroundColor: "#013369", border: "1px solid #9ca3af", borderRadius: 6, boxShadow: "0 8px 24px rgba(0,0,0,0.45)" }}>
          ...
          <thead>
            <tr>
              {["Sample Size", "% Beats Vegas", "Wagered", "Won", "ROI"].map((h) => (
                <th key={h} style={{ backgroundColor: "#013369", color: "#ffffff", padding: "7px 10px", textAlign: "center", fontSize: "0.68rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", borderBottom: "1px solid rgba(255,255,255,0.2)", whiteSpace: "nowrap" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
        ...
        <table style={{ borderCollapse: "collapse", width: "100%", minWidth: 700 }}>
          ...
          <tbody>
            {[...rows].reverse().map((row, idx) => {
              const atsWinColor = row.atsWinPct >= 55 ? "#013369" : row.atsWinPct >= 50 ? "#78716c" : "#dc2626";
              const atsRoiColor = row.atsRoi >= 0 ? "#013369" : "#dc2626";
              const ouWinColor = row.ouWinPct >= 55 ? "#013369" : row.ouWinPct >= 50 ? "#78716c" : "#dc2626";
              const ouRoiColor = row.ouRoi >= 0 ? "#013369" : "#dc2626";
              return (
          ...
          <tfoot>
            <tr style={{ backgroundColor: "#f1f5f9", borderTop: "2px solid #cbd5e1" }}>
              ...
              <td style={{ ...cellStyle, fontWeight: 700, color: totalWinPct >= 50 ? "#013369" : "#dc2626", borderBottom: "none" }}>{totalWinPct.toFixed(1)}%</td>
              <td style={{ ...cellStyle, fontWeight: 700, color: totalRoi >= 0 ? "#013369" : "#dc2626", borderBottom: "none" }}>{totalRoi >= 0 ? "+" : ""}{totalRoi.toFixed(1)}%</td>
              {(() => {
                ...
                return (
                  <>
                    ...
                    <td style={{ ...cellStyle, fontWeight: 700, color: ouTotalWinPct >= 50 ? "#013369" : "#dc2626", borderBottom: "none" }}>{ouTotalWinPct.toFixed(1)}%</td>
                    <td style={{ ...cellStyle, fontWeight: 700, color: ouTotalRoi >= 0 ? "#013369" : "#dc2626", borderBottom: "none" }}>{ouTotalRoi >= 0 ? "+" : ""}{ouTotalRoi.toFixed(1)}%</td>
                  </>
                );
              })()}
            </tr>
...
function PageExplainer() {
  ...
  const numStyle: React.CSSProperties = { width: 26, height: 26, borderRadius: "50%", backgroundColor: "#013369", color: "white", fontSize: "0.7rem", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 };
...
export default function NFLAccuracyPage() {
  // Remap nfl-games.json field names to the shared HistoricalGame schema
  const remappedGames: HistoricalGame[] = (games as Record<string, unknown>[]).map((g) => {
    const vegasTotal = g.vegasTotal as number | null;
    const bbmiTotal = g.bbmiTotal as number | null;
    const actualHome = g.actualHomeScore as number | null;
    const actualAway = g.actualAwayScore as number | null;
    const actualTotal = (actualHome != null && actualAway != null) ? actualHome + actualAway : null;
    // Derive totalPick and totalResult from raw projections
    const totalPick = (bbmiTotal != null && vegasTotal != null && bbmiTotal !== vegasTotal)
      ? (bbmiTotal < vegasTotal ? "under" : "over") : null;
    const totalResult = (actualTotal != null && vegasTotal != null && actualTotal !== vegasTotal)
      ? (actualTotal < vegasTotal ? "under" : "over") : null;
    return {
      date: (g.date ?? null) as string | null,
      home: (g.homeTeam ?? null) as string | null,
      away: (g.awayTeam ?? null) as string | null,
      vegasHomeLine: (g.vegasSpread ?? null) as number | null,
      bbmiHomeLine: (g.bbmiSpread ?? null) as number | null,
  ...
  const ouEdgeStats = useMemo(() => {
    const decided = bettableGames.filter(
      (g) =>
        ...
        Math.abs((g.bbmiTotal ?? 0) - (g.vegasTotal ?? 0)) <= MAX_EDGE_FOR_RECORD
    );
    const allWins = decided.filter((g) => g.totalPick === g.totalResult).length;
    const overallWinPct = decided.length > 0 ? ((allWins / decided.length) * 100).toFixed(1) : "0.0";
    const highEdge = decided.filter((g) => Math.abs((g.bbmiTotal ?? 0) - (g.vegasTotal ?? 0)) >= 4.0);
    const highEdgeWins = highEdge.filter((g) => g.totalPick === g.totalResult).length;
    const highEdgeWinPct = highEdge.length > 0 ? ((highEdgeWins / highEdge.length) * 100).toFixed(1) : "0.0";
    const eliteEdge = decided.filter((g) => Math.abs((g.bbmiTotal ?? 0) - (g.vegasTotal ?? 0)) >= FREE_EDGE_LIMIT);
  ...
  const edgeOptions = useMemo(() => mode === "ou" ? [0, 2.5, 4, 5.5] : [0, 2, 3, 4, 5, 6, 7], [mode]);
  ...
  const edgeFilteredGames = useMemo(() => {
    if (mode === "ou") {
      const ouGames = bettableGames.filter((g) => g.vegasTotal != null && g.bbmiTotal != null && Math.abs((g.bbmiTotal ?? 0) - (g.vegasTotal ?? 0)) <= MAX_EDGE_FOR_RECORD);
  ...
  const getTeamRecord = (teamName: string) => {
    ...
    return { wins: record.wins, picks: record.picks, display: `${record.wins}-${record.picks - record.wins}`, color: record.wins / record.picks >= 0.5 ? "#013369" : "#dc2626" };
  };
  ...
  const activeEdgeLimit = mode === "ats" ? FREE_EDGE_LIMIT : FREE_EDGE_LIMIT;
  ...
  const edgeCellStyle = (edge: number): React.CSSProperties => {
    ...
    return {
      ...
      color: isHighEdge ? "#92400e" : isBelowMin ? "#b0b8c1" : "#013369",
      ...
      <div className="section-wrapper" style={{ backgroundColor: "#f0efe9" }}>
        <div className="w-full max-w-[1100px] mx-auto px-6 py-8">
          ...
          <div style={{ textAlign: "center", borderBottom: "1px solid #d4d2cc", paddingBottom: 20, marginBottom: 24 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, backgroundColor: "#013369", color: "#fff", borderRadius: 999, padding: "5px 14px", fontSize: 11, fontWeight: 600, marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 13, color: "#666", margin: "0 auto 14px", lineHeight: 1.6 }}>Every NFL totals pick tracked transparently {"\u2014"} no retroactive edits</p>
            <div style={{ display: "flex", gap: 4, justifyContent: "center" }}>
              {(["ou", "ats"] as const).map((m) => (
                <button key={m} onClick={() => { setMode(m); if (m === "ou" && minEdge > 3) setMinEdge(0); }} style={{
                  ...
                  backgroundColor: mode === m ? "#013369" : "transparent",
          ...
          {(() => {
            ...
            const hThresh = mode === "ou" ? 4.0 : FREE_EDGE_LIMIT;
            const eThresh = mode === "ou" ? FREE_EDGE_LIMIT : 10;
            return (
              <div style={{ maxWidth: 1100, margin: "0 auto 2rem", display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: "0.75rem" }}>
                {[
                  ...
                  <div key={card.label} style={{
                    ...
                    border: card.premium ? "2px solid #013369" : "1px solid #d4d2cc",
                    borderTop: "4px solid #013369", borderRadius: 10,
                    padding: "14px 14px 12px", textAlign: "center",
                  }}>
                    <div style={{ fontSize: card.premium ? 28 : 24, fontWeight: card.premium ? 700 : 500, color: "#013369", lineHeight: 1.1 }}>{card.value}</div>
                    <div style={{ fontSize: "0.68rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.07em", color: card.premium ? "#013369" : "#777", margin: "4px 0 3px" }}>{card.label}</div>
          ...
          <div style={{ maxWidth: 1100, margin: "-1.5rem auto 2rem" }}>
            <button
              ...
              style={{
                ...
                backgroundColor: "#013369", color: "#94a3b8",
            ...
            >
              <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ color: "#013369", fontSize: 14 }}>&#10003;</span>
            ...
            {walkForwardOpen && (
              <div style={{ backgroundColor: "#013369", borderRadius: "0 0 10px 10px", border: "1px solid #013369", borderTop: "1px solid rgba(255,255,255,0.08)", overflow: "hidden" }}>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        border: isActive ? "2px solid #013369" : "2px solid #d6d3d1",
                        backgroundColor: isActive ? "#013369" : "#ffffff",
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <input type="text" placeholder="Search team name..." value={teamSearch} autoComplete="off"
                    ...
                    style={{
                      ...
                      border: selectedTeam ? "1.5px solid #013369" : "1.5px solid #d6d3d1",
                  ...
                  <div style={{ position: "absolute", width: "100%", marginTop: 4, backgroundColor: "#ffffff", border: "2px solid #d6d3d1", borderRadius: 6, boxShadow: "0 8px 24px rgba(0,0,0,0.12)", maxHeight: 240, overflowY: "auto", zIndex: 999999 }}>
                    {filteredTeams.map((team) => (
                      <div key={team} onMouseDown={(e) => { e.preventDefault(); handleTeamSelect(team); }}
                        ...
                        <NFLLogo team={team} size={20} /><span>{team}</span>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            <p style={{ fontSize: 12, color: "#78716c", fontStyle: "italic" }}>
              ...
              {minEdge >= activeEdgeLimit && <span style={{ color: "#013369", fontWeight: 700 }}> ✓ You&apos;re viewing high-edge picks — {activeEdgeStats.highEdgeWinPct}% accuracy at this threshold.</span>}
            </p>
          </div>

          {selectedTeam && (
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 24 }}>
              <div style={{ backgroundColor: "#013369", color: "#ffffff", padding: "8px 16px", borderRadius: 8, display: "flex", alignItems: "center", gap: 8 }}>
                <NFLLogo team={selectedTeam} size={24} />
          ...
          <SummaryCard
            ...
            data={summary}
            wins={wins}
            colors={{ winPct: Number(summary.bbmiWinPct) > 50 ? "#013369" : "#dc2626", won: summary.fakeWon > summary.fakeWagered ? "#013369" : "#dc2626", roi: Number(summary.roi) > 0 ? "#013369" : "#dc2626" }}
          />
                    ...
                    <tbody>
                      {displayedTeams.map((td, idx) => (
                        <tr key={td.team} style={{ backgroundColor: idx % 2 === 0 ? "#ffffff" : "#f9fafb" }}>
                          ...
                          <td style={TD}>
                            <Link href={`/nfl/team/${encodeURIComponent(td.team)}`} style={{ display: "flex", alignItems: "center", gap: 8, color: "#1a1a1a", fontWeight: 600, fontSize: 13 }} className="hover:underline">
                              <NFLLogo team={td.team} size={22} />{td.team}
                            </Link>
                          </td>
                          <td style={TD_CENTER}>{td.games.toLocaleString()}</td>
                          <td style={{ ...TD_CENTER, fontWeight: 700, color: td.winPct >= 50 ? "#013369" : "#dc2626" }}>{td.winPct.toFixed(1)}%</td>
                          <td style={{ ...TD_CENTER, color: "#dc2626", fontFamily: "ui-monospace, monospace" }}>${td.wagered.toLocaleString()}</td>
                          <td style={{ ...TD_CENTER, fontFamily: "ui-monospace, monospace", color: td.won >= td.wagered ? "#013369" : "#dc2626" }}>${td.won.toLocaleString()}</td>
                          <td style={{ ...TD_CENTER, fontWeight: 700, color: td.roi >= 0 ? "#013369" : "#dc2626" }}>{td.roi.toFixed(1)}%</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
          ...
          <SummaryCard
            ...
            data={weeklySummary}
            wins={weeklyWins}
            colors={{ winPct: Number(weeklySummary.bbmiWinPct) > 50 ? "#013369" : "#dc2626", won: weeklySummary.fakeWon > weeklySummary.fakeWagered ? "#013369" : "#dc2626", roi: Number(weeklySummary.roi) > 0 ? "#013369" : "#dc2626" }}
          />
                    ...
                    {mode === "ats" ? (
                      <>
                        <tr>
                          ...
                          <th colSpan={2} style={{ backgroundColor: "#013369", color: "#ffffff", padding: "8px 10px", textAlign: "center", fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", whiteSpace: "nowrap", position: "sticky", top: 0, zIndex: 20, borderBottom: "1px solid rgba(255,255,255,0.25)" }}>
                            Home Line
                          </th>
                    ...
                    {sortedHistorical.map((g, i) => {
                      ...
                      return (
                        <tr key={i} style={getRowStyle(g.edge, i)}>
                          ...
                          <td style={rowTD}>
                            <Link href={`/nfl/team/${encodeURIComponent(String(g.away))}`} style={{ display: "flex", alignItems: "center", gap: 6, color: isBelowMin ? "#9ca3af" : "#1a1a1a" }} className="hover:underline">
                              <NFLLogo team={String(g.away)} size={22} />
                          ...
                          <td style={rowTD}>
                            <Link href={`/nfl/team/${encodeURIComponent(String(g.home))}`} style={{ display: "flex", alignItems: "center", gap: 6, color: isBelowMin ? "#9ca3af" : "#1a1a1a" }} className="hover:underline">
                              <NFLLogo team={String(g.home)} size={22} />
                              <div style={{ display: "flex", flexDirection: "column" }}>
                          ...
                          {mode === "ats" ? (
                            <>
                              ...
                              <td style={{ ...rowTDR, fontWeight: 600, color: isBelowMin ? "#9ca3af" : (isWin(g) ? "#013369" : isPush(g) ? "#a8a29e" : "#dc2626") }}>${g.fakeWin}</td>
                            </>
                          ) : (
                            <>
                          ...
                          <td style={rowTDC}>
                            {g.result === "win" ? <span style={{ color: isBelowMin ? "#9ca3af" : "#013369", fontWeight: 900, fontSize: "1.1rem" }}>✓</span>
